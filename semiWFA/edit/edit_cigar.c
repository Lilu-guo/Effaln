    ...
    edit_cigar_t *const edit_cigar)
{
  ...
  while (p < pattern_length)
  {
    edit_cigar->operations[i++] = 'D';
    ++p;
  };
  while (t < text_length)
  {
    edit_cigar->operations[i++] = 'I';
    ++t;
  };
    ...
    lineal_penalties_t *const penalties)
{
  ...
  for (i = edit_cigar->begin_offset; i < edit_cigar->end_offset; ++i)
  {
    ...
    case 'X':
      ...
      break;
    case 'I':
      score -= penalties->insertion;
      break;
    case 'D':
      score -= penalties->deletion;
      break;
    default:
      return INT_MIN;
    }
...
int edit_cigar_score_gap_affine(
    edit_cigar_t *const edit_cigar, affine_penalties_t *const penalties)
{
  char last_op = '\0';
  int score = 0, i;
  for (i = edit_cigar->begin_offset; i < edit_cigar->end_offset; ++i)
    ...
    edit_cigar_t *const edit_cigar_b)
{
  ...
  if (length_cigar_a != length_cigar_b)
    return length_cigar_a - length_cigar_b;
...
void edit_cigar_print_pretty(
    FILE *const stream, const char *const pattern,
    const int pattern_length,
    const char *const text,
    const int text_length,
    edit_cigar_t *const edit_cigar, mm_allocator_t *const mm_allocator)
{
  char *const operations = edit_cigar->operations;
