						 ...
						 verbose_(verbose)
{
...
#else
		cerr << "Shouldn't be at " << __FILE__ << ":" << __LINE__ << " without _MM defined" << endl;
		throw 1;
#endif
	}
	else
	...
	if (sanity_)
	{
		...
		EList<SString<char>> osv(DEBUG_CAT);
		EList<SString<char>> osn(DEBUG_CAT);
		EList<size_t> osvLen(DEBUG_CAT);
		EList<size_t> osnLen(DEBUG_CAT);
...
{
	uint64_t reci = refRecOffs_[tidx];
	uint64_t recf = refRecOffs_[tidx + 1];
	assert_gt(recf, reci);
	...
	bool sanity)
{
	...
	if (!outfile.empty())
	{
		...
		writeU<int32_t>(fout3, 1, bigEndian);
		...
		if (color)
		{
			...
			parms.color = true;
			writeU<TIndexOffU>(fout3, (TIndexOffU)szs.size(), bigEndian);
