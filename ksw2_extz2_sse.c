  ...
  for (r = 0, last_st = last_en = -1; r < qlen + tlen - 1; ++r)
  {
...
#ifdef __SSE4_1__
        z = _mm_max_epi8(z, a);
#else z = _mm_and_si128(z, _mm_cmpgt_epi8(z, zero_)); z = _mm_max_epu8(z, a); #endif
        __dp_code_block2;
#ifdef __SSE4_1__
...
#else z = _mm_and_si128(z, _mm_cmpgt_epi8(z, zero_)); z = _mm_max_epu8(z, a); tmp = _mm_cmpgt_epi8(b, z);
    ...
    else
    {
      ...
      for (t = st_; t <= en_; ++t)
      {
        __m128i d, z, a, b, xt1, vt1, ut, tmp;
        __dp_code_block1;
        d = _mm_andnot_si128(_mm_cmpgt_epi8(z, a), flag1_);
#ifdef __SSE4_1__
        z = _mm_max_epi8(z, a);
        tmp = _mm_cmpgt_epi8(z, b);
        d = _mm_blendv_epi8(flag2_, d, tmp);
#else z = _mm_and_si128(z, _mm_cmpgt_epi8(z, zero_)); z = _mm_max_epu8(z, a); tmp = _mm_cmpgt_epi8(z, b);
        d = _mm_or_si128(_mm_andnot_si128(tmp, flag2_), _mm_and_si128(tmp, d));
#endif
        __dp_code_block2;
    ...
    if (!approx_max)
    {
      ...
      if (r > 0)
      {
        ...
        H[0] = v8[0] - qe - qe, max_H = H[0], max_t = 0;
